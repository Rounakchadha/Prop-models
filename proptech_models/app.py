...
def roi_heatmap():
    ...
    try:
        ...
        if not heatmap_data:
            flash("No data available for heatmap generation", "error")
...
def price_prediction():
    """Price prediction using your price forecasting models"""
    if not PRICE_MODEL_AVAILABLE:
        flash("Price prediction models not available", "error")
        return redirect(url_for('home'))
    ...
    if request.method == "POST":
        try:
            ...
            prediction_months = int(request.form.get("prediction_months", 12))
            
            # Use your price prediction models
            if 'price_forecaster' in ml_service.tools:
                price_forecast = ml_service.tools['price_forecaster'].predict_price(
                    locality, current_price, prediction_months
                )
            else:
                price_forecast = {"error": "Price forecaster not available"}
            
            if "error" in price_forecast:
                flash(price_forecast["error"], "error")
                return redirect(url_for('price_prediction'))
            
            return render_template("price_prediction_result.html", 
                                 forecast=price_forecast)
            
        except ValueError:
            flash("Please enter valid numeric values", "error")
        except Exception as e:
            flash(f"Error in price prediction: {str(e)}", "error")
...
def api_predict_roi():
    """API endpoint for ROI prediction"""
    try:
        data = request.get_json()
        locality = data.get('locality', '').lower().strip()
        price = float(data.get('price', 0))
        
        if ROI_MODEL_AVAILABLE:
            prediction = predict_roi(locality, price * 100000)  # Convert to actual amount
        else:
            prediction = ml_service.calculate_roi_fallback(locality, price)
        
        if prediction is None:
            return jsonify({"error": "Unable to predict ROI for this locality"}), 400
        
        return jsonify({
            "locality": locality.title(),
            "price": price,
            "predicted_roi": round(prediction, 2),
            "method": "ML Model" if ROI_MODEL_AVAILABLE else "Statistical"
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
def calculate_realistic_risk_score(self, stats, monthly_cash_flow, roi, risk_tolerance, property_price):
    ...
    # 1. Cash Flow Risk (35% weight) - Less penalty for negative cash flow
    if monthly_cash_flow >= 5000:
        cash_flow_risk = 0
    elif monthly_cash_flow >= 0:
        cash_flow_risk = 5
    elif monthly_cash_flow >= -5000:
        cash_flow_risk = 15  # Reduced penalty
...
def get_realistic_investment_recommendation(self, roi, monthly_cash_flow, risk_score):
    ...
    # Good investments (accepting some negative cash flow)
    elif roi >= 8 and monthly_cash_flow >= -5000 and risk_score < 45:
        return "Recommended"
    
    # Acceptable investments (capital appreciation focus)
...
def not_found_error(error):
    return render_template('404.html') if os.path.exists('template/404.html') else "Page not found", 404

@app.errorhandler(500)
def internal_error(error):
    return render_template('500.html') if os.path.exists('template/500.html') else "Internal server error", 500
...
def system_info():
    ...
    return jsonify({
        "available_models": {
            "roi_calculator": ROI_MODEL_AVAILABLE,
            "market_comparison": MARKET_COMPARISON_AVAILABLE,
            "investment_calculator": INVESTMENT_CALCULATOR_AVAILABLE,
            "price_prediction": PRICE_MODEL_AVAILABLE,
            "heatmap_generator": HEATMAP_AVAILABLE
        },
        "data_statistics": {
            "total_localities": len(ml_service.data.get('localities', [])),
            "data_sources": ["Final_Project.csv", "Mumbai_House_Rent.csv"],
...
if __name__ == "__main__":
    ...
    print("📊 API info: http://localhost:5001/api/system-info")
    print("=" * 50)
